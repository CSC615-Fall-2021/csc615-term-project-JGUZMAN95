...
# * File: PCA9685.c
# *
# * Description: PCA9685 'C' file that implements the functions
# * declared in the PCA9685 header file that will help us run, stop,
# * and control the speed of our motor.
# **************************************************************/
 ...
 */
static void PCA9685_WriteByte(UBYTE reg, UBYTE value,UBYTE value1)
{
    DEV_I2C_WriteByte(reg, value, value1);
}
...
static UBYTE PCA9685_ReadByte1(UBYTE reg)
{
    return DEV_I2C_ReadByte1(reg);
...
{
    PCA9685_WriteByte(LED0_ON_L + 4*channel, on & 0xFF, on & 0xFF);
    PCA9685_WriteByte(LED0_ON_H + 4*channel, on >> 8, on >> 8);
    PCA9685_WriteByte(LED0_OFF_L + 4*channel, off & 0xFF, off & 0xFF);
    PCA9685_WriteByte(LED0_OFF_H + 4*channel, off >> 8,off >> 8);
}
 ...
 * PCA9685_Init(0x40);
 */
void PCA9685_Init(char addr, char addr1)
{
    DEV_I2C_Init(addr, addr1);
    DEV_I2C_WriteByte(MODE1, 0x00, 0x00);
}
...
void PCA9685_SetPWMFreq(UWORD freq)
{
    ...
    UBYTE oldmode1 = PCA9685_ReadByte1(MODE1);

    UBYTE newmode = (oldmode & 0x7F) | 0x10; // sleep
    UBYTE newmode1 = (oldmode1 & 0x7F) | 0x10; // sleep


    PCA9685_WriteByte(MODE1, newmode, newmode1); // go to sleep

    PCA9685_WriteByte(PRESCALE, prescale, prescale); // set the prescaler
    PCA9685_WriteByte(MODE1, oldmode, oldmode1);

    DEV_Delay_ms(5);
    PCA9685_WriteByte(MODE1, oldmode | 0x80,oldmode | 0x80 );  //  This sets the MODE1 register to turn on auto increment.
    PCA9685_WriteByte(MODE1, oldmode1 | 0x80,oldmode1 | 0x80 );  //  This sets the MODE1 register to turn on auto increment.
