...
# * File: DEV_Config.c
# *
# * Description: 'C' file that has functions that are used in the
# * PCA9685.c for certain functions. Used to init the controller
# * and to read/write bytes.
# **************************************************************/
 ...
 * DEV_I2C_Init(0X57);
 */
void DEV_I2C_Init(char addr, char addr1)
{
    fd = wiringPiI2CSetup(addr);
    fd1 = wiringPiI2CSetup(addr1);
}
 ...
 */
void DEV_I2C_WriteByte(UBYTE reg, UBYTE value, UBYTE value1)
{
    int ref;
    int ref1;

    ref = wiringPiI2CWriteReg8(fd, (int)reg, (int)value);
    ref1 = wiringPiI2CWriteReg8(fd1, (int)reg, (int)value1);
    while(ref != 0 && ref1 != 0) {
        ref = wiringPiI2CWriteReg8 (fd, (int)reg, (int)value);
        ref1 = wiringPiI2CWriteReg8 (fd1, (int)reg, (int)value1);
        if(ref == 0 && ref1 == 0){
...
UBYTE DEV_I2C_ReadByte1(UBYTE reg1)
{
    return wiringPiI2CReadReg8(fd1,reg1);
}
